...
var player,playerimg,forwardimg,backward;
...
function preload(){
    ...
    playerimg=loadAnimation("dave pc(red1).png");
    forwardimg=loadAnimation("dave pc(red2).png","dave pc(red1).png","dave pc(red2).png");

    backward=loadAnimation("dave pc(red-2).png","dave pc(red(-1).png")
...
function setup(){
   ...
   // console.log(height);
    ...
    vd1=createSprite(width-700,height-200,10,10);
    vd1.addImage(vdimg);
    vd1.scale=0.5
    ...
    player=createSprite(width-1275,height-95,10,10);
    player.addAnimation("hi",playerimg);
    player.addAnimation("move",forwardimg);
    player.addAnimation("back",backward);

    player.depth=pole.depth;
...
function draw(){
    ...
    if(keyWentDown("right")){
       player.changeAnimation("move",forwardimg);
       player.x=player.x+3; 
     }
     if(keyWentUp("right")){
        player.changeAnimation("hi",playerimg); 
      }

      if(keyWentDown("left")){
        player.changeAnimation("back",backimg);
